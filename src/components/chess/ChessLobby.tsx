...
import type { ChessLobbyEntry, ChessView } from "@/lib/games/chess/engine";
import type { Wallet } from "@/lib/wallet";
import type { User } from "@/lib/auth";
import { countdown, formatCents, relativeTime } from "@/lib/format";

export type ChessLobbyState = {
  now: number;
  user: User | null;
  wallet: Wallet | null;
  challenges: ChessLobbyEntry[];
  mine: ChessView[];
  config: {
...
export function ChessLobby({ initial }: { initial: ChessLobbyState }) {
  const router = useRouter();
  const [state, setState] = useState(initial);
  const [stake, setStake] = useState(
    initial.config.stakePresets.find((p) => p.cents === 50_000)?.cents ??
      initial.config.stakePresets[0].cents,
  );
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/games/chess/state", { cache: "no-store" });
  ...
  // Someone accepting your challenge drops you onto the board.
  useEffect(() => {
    const justStarted = state.mine.find(
      (m) => m.status === "active" && m.startedAt && Date.now() - m.startedAt < 6000,
    );
    if (justStarted) router.push(`/games/chess/${justStarted.id}`);
  }, [state.mine, router]);

  async function createChallenge() {
    if (!state.user) {
      router.push("/join?next=/games/chess");
      return;
    }
    setBusy("create");
    setError(null);
    try {
      ...
      router.push(`/games/chess/${data.match.id}`);
    } catch {
  ...
  async function accept(entry: ChessLobbyEntry) {
    if (!state.user) {
      router.push(`/join?next=/games/chess/${entry.id}`);
      return;
    }
    setBusy(entry.id);
    setError(null);
    try {
      const res = await fetch(`/api/games/chess/matches/${entry.id}`, {
        method: "POST",
        headers: { "content-type": "application/json" },
      ...
      router.push(`/games/chess/${entry.id}`);
    } catch {
      setError("Network hiccup — try again.");
    } finally {
      setBusy(null);
  ...
  const timeControl = `${Math.round(state.config.baseSeconds / 60)}+${state.config.incrementSeconds}`;
      ...
      <header className="flex flex-wrap items-end justify-between gap-6">
        <div>
          <div className="inline-flex items-center gap-2 rounded-full border border-line bg-panel/70 px-3.5 py-1.5 text-xs text-muted">
            ...
            {timeControl} blitz · winner takes the pot
          </div>
          <h1 className="display display-hero mt-5 text-5xl sm:text-6xl">
            Chess <span className="text-lime">Stakes</span>
          </h1>
          <p className="mt-4 max-w-xl text-muted">
            Real chess, real clock, real escrow. Colours are drawn at random when someone accepts.
          </p>
        </div>
        {state.wallet && (
            ...
            {live.map((m) => (
              ...
              >
                ...
                <div className="tabular mt-1 text-xs text-dim">
                  {m.status === "active"
                    ? `${m.yourTurn ? "your move" : "their move"} · ${countdown(m.you?.clockMs ?? 0)}`
      ...
      <div className="mt-10 grid gap-6 lg:grid-cols-[minmax(0,1fr)_340px]">
        <div className="space-y-6">
          <section className="panel px-6 py-7">
            ...
            <p className="mt-2 text-sm text-muted">
              {timeControl} blitz. Your stake sits in escrow until someone accepts — cancel before that and
            ...
            <p className="mt-3 text-xs text-dim">
              House fee {(state.config.rakeBps / 100).toFixed(1)}% ({formatCents(selected.rake)}). Any draw —
              stalemate, repetition, insufficient material — returns both stakes in full.
            </p>
        ...
        <aside className="space-y-6">
          <section className="panel px-6 py-6">
            <h3 className="display text-xl">House rules</h3>
            <ul className="mt-4 space-y-3 text-sm text-muted">
              {[
                `${timeControl} blitz — ${Math.round(state.config.baseSeconds / 60)} minutes each, ${state.config.incrementSeconds}s added per move`,
                "Colours are drawn at random when the challenge is accepted",
                "Checkmate, resignation or a flag on time wins the pot",
                "Stalemate, repetition, insufficient material and the fifty-move rule all refund both stakes",
                "Every move is validated on the server — the client never decides legality",
              ].map((rule) => (
          ...
          {state.mine.filter((m) => m.status === "finished").length > 0 && (
            <section className="panel overflow-hidden">
              <h3 className="display border-b border-line-soft px-5 py-4 text-xl">Recent games</h3>
              <ul className="divide-y divide-line-soft">
                {state.mine
                  ...
                  .map((m) => {
                    ...
                    return (
                      <li key={m.id}>
                        <Link href={`/games/chess/${m.id}`} className="flex items-center gap-3 px-5 py-3.5">
...
function Figure({ label, value, accent }: { label: string; value: string; accent?: boolean }) {
  return (
    ...
    >
      <div className={`text-xs uppercase tracking-widest ${accent ? "opacity-70" : "text-dim"}`}>{label}</div>
